    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.crm-customers.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('facebook_profile') ? 'is-invalid' : '' }}" type="text" name="facebook_profile" id="facebook_profile" value="{{ old('facebook_profile', 'https://facebook.com/') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('whatsapp') ? 'is-invalid' : '' }}" name="whatsapp" id="whatsapp">
                    ...
                    @foreach(App\Models\CrmCustomer::WHATSAPP_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('whatsapp', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
